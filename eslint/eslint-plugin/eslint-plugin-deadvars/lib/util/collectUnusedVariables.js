...
class UnusedVarsVisitor extends Visitor_1.Visitor {
    ...
    static collectUnusedVariables(context) {
        ...
        const unusedVars = visitor.collectUnusedVariables(visitor.getScope(program), undefined, context);
        this.RESULTS_CACHE.set(program, unusedVars);
        return unusedVars;
    }
    collectUnusedVariables(scope, unusedVariables = new Set(), context) {
        // 添加逻辑：已使用变量需排除掉ts-unused-exports检测出来的变量
        const firstOption = context.options[0];
        let varsIgnorePattern;
        if (firstOption && firstOption.filenameArr) {
            const filename = context.getFilename();
            const filenameOptions = firstOption.filenameArr.find((filenameItem) => filenameItem.filename === filename);
            if (filenameOptions) {
                varsIgnorePattern = new RegExp(filenameOptions.varsIgnorePattern, 'u');
            }
        }
        for (const variable of scope.variables) {
            const isVarExported = isExported(variable);
            if (
                !isVarExported &&
            ...
            if (isVarExported && (!varsIgnorePattern || !varsIgnorePattern.test(variable.name))) {
                continue;
            }
            unusedVariables.add(variable);
        }
        for (const childScope of scope.childScopes) {
            this.collectUnusedVariables(childScope, unusedVariables, context);
        }
